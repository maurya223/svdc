...
# Expose the local server to the internet using SSH tunneling with Serveo.net
print("Exposing server to the internet via SSH tunneling (Serveo.net)...")
tunnel_process = subprocess.Popen(['ssh', '-R', '80:localhost:8000', 'serveo.net'], stdout=subprocess.PIPE, stderr=subprocess.PIPE)

# Read the tunnel URL from stdout
for line in iter(tunnel_process.stdout.readline, b''):
    line = line.decode('utf-8').strip()
    if 'Forwarding HTTP traffic from' in line:
        public_url = line.split('Forwarding HTTP traffic from')[1].strip()
